...
// strictBashSpec returns the sandbox spec for strict plan execution: the
// workspace stays locked by forbidding reads of the Reasonix home (master key
// location) and the plan artifact directory. On OS-sandbox platforms (Linux
// bubblewrap / macOS Seatbelt) these become actual read denials; on Windows the
// fields still ride the spec and the documented OS-level limitation applies
// (see docs/arch/plan-execute.md §7).
func strictBashSpec(spec sandbox.Spec, workDir string) sandbox.Spec {
	spec.ForbidReadRoots = appendUniqueRoots(spec.ForbidReadRoots,
		planstore.HomeDir(), planstore.PlansDir(workDir))
	return spec
}

func appendUniqueRoots(roots []string, extra ...string) []string {
	...
	for _, e := range extra {
		...
		dup := false
		for _, r := range out {
			if strings.EqualFold(filepath.Clean(r), filepath.Clean(e)) {
				dup = true
				break
			}
		}
...
func (b bash) ExecuteDetailed(ctx context.Context, args json.RawMessage) (tool.DetailedResult, error) {
	start := time.Now()
	// Strict plan execution: lock the workspace before launch — forbid reads of
	// the Reasonix home (master key) and the plan artifact directory. b is a
	// value receiver, so the override is local to this call.
	if planmode.StrictActive(ctx) {
		b.sb = strictBashSpec(b.sb, b.workDir)
	}
