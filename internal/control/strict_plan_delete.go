...
func (c *Controller) applyStrictPlanDelete(input, display string) {
	...
	if id == "" {
		c.strictPlanNotice("usage: /strict-plan-delete <plan-id> — 必须显式指定计划 id 才能删除（删除不可恢复，省略 id 不兜底最近计划）")
		return
	}
	c.runGuarded(func(ctx context.Context) error {
		store, err := planstore.Open(c.workspaceRoot)
		if err != nil {
			c.strictPlanNotice("strict-plan-delete: " + err.Error())
			return nil
		}
...
func (c *Controller) deletePlan(store *planstore.Store, id string) error {
	...
	if _, err := os.Stat(dir); err != nil {
		c.strictPlanNotice(fmt.Sprintf("strict-plan-delete: plan %q 不存在或不可读（%v）", id, err))
		return nil
	}
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			...
			c.strictPlanNotice(fmt.Sprintf("strict-plan-delete: plan %q 运行状态缺失（残留计划），仍执行删除", id))
		} else {
			c.strictPlanNotice(fmt.Sprintf("strict-plan-delete: plan %q 运行状态不可读（%v），拒绝删除（无法确认其未在执行中）", id, err))
			return nil
		}
	} else if rs.Stage == planstore.StageExecuting {
		c.strictPlanNotice(fmt.Sprintf("strict-plan-delete: plan %q 正在执行中（stage executing），禁止删除；请等待执行结束（done/failed）后再删除", id))
		return nil
	}
	...
	c.strictPlanNotice(fmt.Sprintf("strict-plan-delete: plan %q 已删除（不可恢复）", id))
	return nil
}
