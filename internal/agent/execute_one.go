...
func (a *Agent) resolveToolPolicy(ctx context.Context, plan *toolCallPlan) (toolOutcome, bool) {
	...
	// Strict plan execution: hold file-mutation tools to the locked plan's
	// write scope before the ordinary permission path.
	if blocked, early := a.applyPlanWriteScope(ctx, plan); early {
		return blocked, true
	}
...
// applyPlanWriteScope enforces the locked plan's write scope during strict plan
// execution: file-mutation tools targeting paths outside the whitelist are
// refused before the ordinary permission path. The executing plan's scope comes
// from the encrypted store; a strict loop without a bound store or executing
// plan fails closed (no writes at all).
func (a *Agent) applyPlanWriteScope(ctx context.Context, plan *toolCallPlan) (toolOutcome, bool) {
	if !planmode.StrictActive(ctx) {
		return toolOutcome{}, false
	}
	s, ok := planstore.StoreFromContext(ctx)
	if !ok {
		return toolOutcome{
			output:  "blocked: strict plan execution requires a plan store bound to the context",
			blocked: true,
			errMsg:  "blocked: plan store unavailable",
		}, true
	}
	id, err := activeExecutingPlan(s)
	if err != nil {
		return toolOutcome{
			output:  "blocked: " + err.Error(),
			blocked: true,
			errMsg:  "blocked: no executing plan",
		}, true
	}
	files, err := s.ReadPlan(id)
	if err != nil {
		return toolOutcome{
			output:  fmt.Sprintf("blocked: cannot load plan %q for write-scope enforcement: %v", id, err),
			blocked: true,
			errMsg:  "blocked: plan unreadable",
		}, true
	}
	canonical := plan.canonicalName
	if plan.resolvedMeta != nil && plan.resolvedMeta.TargetName != "" {
		canonical = plan.resolvedMeta.TargetName
	}
	gate := planstore.WriteGate{WSRoot: s.WorkspaceRoot(), Scope: planstore.Scope(files.Scope)}
	if err := gate.Check(canonical, plan.execArgs); err != nil {
		return toolOutcome{
			output:  err.Error(),
			blocked: true,
			errMsg:  "blocked: outside the plan write scope",
		}, true
	}
	return toolOutcome{}, false
}

// activeExecutingPlan returns the single plan currently executing in this
// workspace (concurrent execution is forbidden per workspace), or an error.
func activeExecutingPlan(s *planstore.Store) (string, error) {
	ids, err := s.ListPlans()
	if err != nil {
		return "", fmt.Errorf("strict plan write scope: %w", err)
	}
	for _, id := range ids {
	...
	return "", fmt.Errorf("strict plan write scope: no executing plan found in this workspace")
}
