...
interface PlanSubmitArgs {
  plan_id?: string;
  steps?: string[];
  validate_script?: string;
  write_scope?: { path?: string; reason?: string }[];
  change_manifest?: { path?: string; action?: string }[];
}

// PlanSubmitView renders the plan_submit gate payload as a readable card —
// numbered steps, the full acceptance script (real newlines, syntax-highlighted
// via CodeViewer), the write scope with reasons and the change manifest —
// instead of the generic pretty-JSON fallback. This is the dedicated card for
// the strict-plan tool; other tools keep the pretty-JSON rendering.
function PlanSubmitView({ args }: { args: string }) {
  const t = useT();
  let a: PlanSubmitArgs = {};
  try {
    a = JSON.parse(args) as PlanSubmitArgs;
  } catch {
    // fall through to the empty state below
  }
  const steps = Array.isArray(a.steps) ? a.steps : [];
  const script = typeof a.validate_script === "string" ? a.validate_script : "";
  const scope = Array.isArray(a.write_scope) ? a.write_scope : [];
  const manifest = Array.isArray(a.change_manifest) ? a.change_manifest : [];
  return (
    <div className="tool__plan-submit">
      <div className="tool__plan-section">
        <div className="tool__plan-heading">{t("tool.planSteps", { n: steps.length })}</div>
        <ol className="tool__plan-steps">
          {steps.map((s, i) => (
            <li key={i}>{s}</li>
          ))}
        </ol>
      ...
      <div className="tool__plan-section">
        <div className="tool__plan-heading">{t("tool.planScript")}</div>
        {script ? (
          <CodeViewer value={script} language="python" maxHeight={320} />
        ) : (
          <div className="tool__plan-empty">{t("tool.planEmpty")}</div>
        )}
      </div>
      <div className="tool__plan-section">
        <div className="tool__plan-heading">{t("tool.planScope", { n: scope.length })}</div>
        <ul className="tool__plan-list">
          {scope.map((e, i) => (
            <li key={i}>
              <span className="tool__plan-path">{e.path}</span>
              <span className="tool__plan-dash">——</span>
              <span className="tool__plan-reason">{e.reason}</span>
            </li>
          ))}
        </ul>
      </div>
      <div className="tool__plan-section">
        <div className="tool__plan-heading">{t("tool.planManifest", { n: manifest.length })}</div>
        <ul className="tool__plan-list">
          {manifest.map((m, i) => (
            <li key={i}>
              <span className="tool__plan-path">{m.path}</span>
              <span className="tool__plan-action">[{m.action}]</span>
            </li>
          ))}
        </ul>
      </div>
    ...
    <div className={`tool${quiet ? " tool--quiet" : ""}${isSubagent ? " tool--subagent" : ""}${open && hasBody ? " tool--open" : ""}`} data-entrance={item.id} data-shell={isShellCard ? execution?.shell || "bash" : undefined}>
      ...
      <div ref={toolBodyRef} className="tool__body">
        ...
        {!shellPreview && hasArgsOrOutput && (
          <>
            {item.name === "plan_submit" ? (
              <PlanSubmitView args={effectiveArgs} />
            ) : (
